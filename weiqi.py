...
class Application(Tk):
    ...
    def __init__(self, my_mode_num=9):
        ...
        self.positions = [
            [0 for i in range(self.mode_num + 2)] for i in range(self.mode_num + 2)
        ]
        # 初始化棋盘，所有超过边界的值置-1
        ...
        self.photoBD = PhotoImage(
            file="./Pictures/" + "BD" + "-" + str(self.mode_num) + ".png"
        )
        self.photoWD = PhotoImage(
            file="./Pictures/" + "WD" + "-" + str(self.mode_num) + ".png"
        )
        self.photoBU = PhotoImage(
            file="./Pictures/" + "BU" + "-" + str(self.mode_num) + ".png"
        )
        self.photoWU = PhotoImage(
            file="./Pictures/" + "WU" + "-" + str(self.mode_num) + ".png"
        )
        # 用于黑白棋子图片切换的列表
        ...
        self.canvas_bottom = Canvas(
            self, bg="#369", bd=0, width=600 * self.size, height=400 * self.size
        )
        ...
        self.newGameButton1 = Button(
            self,
            text=("十三" if self.mode_num == 9 else "九") + "路棋",
            command=self.newGame1,
        )
        self.newGameButton1.place(x=480 * self.size, y=300 * self.size)
        self.newGameButton2 = Button(
            self,
            text=("十三" if self.mode_num == 19 else "十九") + "路棋",
        ...
        self.canvas_bottom.create_rectangle(
            0 * self.size, 0 * self.size, 400 * self.size, 400 * self.size, fill="#c51"
        )
        # 刻画棋盘线及九个点
        # 先画外框粗线
        self.canvas_bottom.create_rectangle(
            20 * self.size, 20 * self.size, 380 * self.size, 380 * self.size, width=3
        )
        # 棋盘上的九个定位点，以中点为模型，移动位置，以作出其余八个点
        for m in [-1, 0, 1]:
            for n in [-1, 0, 1]:
                self.oringinal = self.canvas_bottom.create_oval(
        ...
        self.pW = self.canvas_bottom.create_image(
            500 * self.size + 11, 65 * self.size, image=self.photoW
        )
        self.pB = self.canvas_bottom.create_image(
            500 * self.size - 11, 65 * self.size, image=self.photoB
        )
    ...
    def shadow(self, event):
        if not self.stop:
            ...
            if (20 * self.size < event.x < 380 * self.size) and (
    ...
    def getDown(self, event):
        if not self.stop:
            ...
            if (
                ...
                dy = (event.y - 20 * self.size) % self.dd
                x = int(
                    (event.x - 20 * self.size - dx) / self.dd + round(dx / self.dd) + 1
                )
                y = int(
                    (event.y - 20 * self.size - dy) / self.dd + round(dy / self.dd) + 1
                )
                # 判断位置是否已经被占据
                if self.positions[y][x] == 0:
                    ...
                    self.canvas_bottom.addtag_withtag(
                        "position" + str(x) + str(y), self.image_added
                    )
                    ...
                    if not self.last_2_positions == self.positions:
                        ...
                        if (
                            len(deadlist) > 0
                            or self.if_dead([[x, y]], self.present + 1, [x, y]) == False
                        ):
                            ...
                            self.canvas_bottom.addtag_withtag(
                                "image", self.image_added_sign
                            )
                            self.canvas_bottom.addtag_withtag(
                                "image_added_sign", self.image_added_sign
                            )
                            if self.present == 0:
                                self.create_pW()
                                self.del_pB()
    ...
    def if_dead(self, deadList, yourChessman, yourPosition):
        ...
        if ([yourPosition[0] + 1, yourPosition[1]] not in deadList) and (
            ...
            midvar = self.if_dead(
                deadList + [[yourPosition[0] + 1, yourPosition[1]]],
                yourChessman,
        ...
        if ([yourPosition[0], yourPosition[1] + 1] not in deadList) and (
            ...
            midvar = self.if_dead(
                deadList + [[yourPosition[0], yourPosition[1] + 1]],
                yourChessman,
    ...
    def get_deadlist(self, x, y):
        deadlist = []
        for i in [-1, 1]:
            if self.positions[y][x + i] == (2 if self.present == 0 else 1) and (
                [x + i, y] not in deadlist
            ):
                killList = self.if_dead(
                    [[x + i, y]], (2 if self.present == 0 else 1), [x + i, y]
                )
                if not killList == False:
                    deadlist += copy.deepcopy(killList)
            if self.positions[y + i][x] == (2 if self.present == 0 else 1) and (
                [x, y + i] not in deadlist
            ):
                killList = self.if_dead(
                    [[x, y + i]], (2 if self.present == 0 else 1), [x, y + i]
                )
                if not killList == False:
                    deadlist += copy.deepcopy(killList)
        return deadlist
    ...
    def recover(self, list_to_recover, b_or_w):
        if len(list_to_recover) > 0:
            for i in range(len(list_to_recover)):
                self.positions[list_to_recover[i][1]][list_to_recover[i][0]] = (
                    b_or_w + 1
                )
                self.image_added = self.canvas_bottom.create_image(
                ...
                self.canvas_bottom.addtag_withtag(
                    "position"
                    + str(list_to_recover[i][0])
                    + str(list_to_recover[i][1]),
                    self.image_added,
                )
    ...
    def kill(self, killList):
        if len(killList) > 0:
            for i in range(len(killList)):
                ...
                self.canvas_bottom.delete(
                    "position" + str(killList[i][0]) + str(killList[i][1])
                )
